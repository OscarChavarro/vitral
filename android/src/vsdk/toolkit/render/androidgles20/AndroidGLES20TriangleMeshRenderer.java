...
public class AndroidGLES20TriangleMeshRenderer extends AndroidGLES20Renderer {
    ...
    /**
    */
    private static void drawMesh(TriangleMesh nMesh, RendererConfiguration q) {
        int index;
        ...
        index = 0;
        vertexDataArray = 
            new float[(nMesh.getNumTriangles()) * 3 * vertexSizeInBytes];
    ...
    private static void drawTriangle(
        ...
        for (int i = 0; i < 3; i++) {
            ...
            vertexDataArray[index] = 
                (float) nMesh.getVertexPositions()[(p * 3) + 1];
            index++;
            vertexDataArray[index] = 
                (float) nMesh.getVertexPositions()[(p * 3) + 2];
            index++;

            vertexDataArray[index] = 
                (float) nMesh.getVertexNormals()[p * 3];
            index++;
            vertexDataArray[index] = 
                (float) nMesh.getVertexNormals()[(p * 3) + 1];
            index++;
            vertexDataArray[index] = 
                (float) nMesh.getVertexNormals()[(p * 3) + 2];
            index++;
            ...
            index++;
            vertexDataArray[index] = 
                (float) nMesh.getVertexPositions()[p * 3 + 1] * 2;
            index++;
        }
    ...
    */
    private static void sendVertexesToDraw(
        float[] vertexDataArray, 
        ...
        switch (drawingMode) {
            case mode3Position3Normal2UV:
                drawVertices3Position3Color3Normal2Uv(
                    verticesBufferedArray, primitive, numberOfElements);
                break;
            case mode3Position3Color:
                drawVertices3Position3Color(
                    verticesBufferedArray, primitive, numberOfElements);
                break;
            default:
                break;
