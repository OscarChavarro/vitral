...
private static void drawCenterNormal(Cone nCone,  float[] vertexDataArray, int index, RendererConfiguration nRendererConfiguration, int capElement) {
	...
	//Putting the coordinates
	vertexDataArray[index] = (float) 0.0f; index++;
	vertexDataArray[index] = (float) 0.0f; index++;
	vertexDataArray[index] = (capElement == BOTTOM_CAP)? 0.0f : (float) (nCone.getHeight()); index++;
	
	vertexDataArray[index] = (float) 1.0f; index++;
	vertexDataArray[index] = (float) 0.0f; index++;
	vertexDataArray[index] = (float) 0.0f; index++;
	...
	//Putting the normals
	vertexDataArray[index] = vertexDataArray[index-6]; index++;
	vertexDataArray[index] = vertexDataArray[index-6]; index++;
	vertexDataArray[index] = vertexDataArray[index-6]+(float)direction/10.0f; index++;
...
/**
 * auxiliary method that calculates the normal for a vertex on the body surface
 * TODO: renaming. Suggestion calculateBodyNormal
 */
private static void drawBodyNormal(Cone nCone, double radius, double height, double theta,float[] vertexDataArray, int index,	RendererConfiguration nRendererConfiguration) {
	...
	//Putting the normals
	vertexDataArray[index] = vertexDataArray[index-6]+(float) normal.x/10; index++;
	vertexDataArray[index] = vertexDataArray[index-6]+(float) normal.y/10; index++;
	vertexDataArray[index] = vertexDataArray[index-6]+(float) normal.z/10; index++;
