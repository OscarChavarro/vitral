...
| 13.6 (wide-sector test) | Implemented (`checkWideness`) with TODO warning | Partial | Replace heuristic with orientation-consistent formal predicate using face normal and signed angle | `PolyhedralBoundedSolidOperator.java` |
...
|---|---|---|---|---|
| Sector wideness and orientation | `checkWideness` uses coarse cross-length threshold and TODO note | Signed-angle classification using local tangent basis and face normal orientation | 1. Build orthonormal frame at vertex from face normal and edge tangents. 2. Compute signed turn angle (`atan2`) robustly. 3. Classify wide/non-wide from angle interval with explicit tolerance policy. 4. Add mirror-symmetry and orientation invariance tests. | Deterministic sector classification independent of incidental vector magnitudes |
...
2. Formal predicate replacement (`checkWideness`, coplanar relation algebra, containment determinism).
