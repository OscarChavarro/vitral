...
export class ByteBuffer {
    ...
    public static allocate(capacity: number): ByteBuffer {
        return ByteBuffer.allocateChecked(capacity);
    }
    ...
    public position(position?: number): number | ByteBuffer {
        ...
        if (!Number.isInteger(position) || position < 0 || position > this.limitValue)
    ...
    public limit(limit?: number): number | ByteBuffer {
        ...
        if (!Number.isInteger(limit) || limit < 0 || limit > this.capacity())
    ...
    public getShort(index?: number): number {
        return this.view().getInt16(index === undefined ? this.take(2) : this.at(index, 2), this.littleEndian());
    }
    public putShort(value: number): ByteBuffer;
    public putShort(index: number, value: number): ByteBuffer;
    public putShort(first: number, second?: number): ByteBuffer {
        this.view().setInt16(
            second === undefined ? this.take(2) : this.at(first, 2),
            second ?? first,
            this.littleEndian(),
        );
        return this;
    ...
    public getInt(index?: number): number {
        return this.view().getInt32(index === undefined ? this.take(4) : this.at(index, 4), this.littleEndian());
    }
    public putInt(value: number): ByteBuffer;
    public putInt(index: number, value: number): ByteBuffer;
    public putInt(first: number, second?: number): ByteBuffer {
        this.view().setInt32(
            second === undefined ? this.take(4) : this.at(first, 4),
            second ?? first,
            this.littleEndian(),
        );
        return this;
    ...
    public getLong(index?: number): bigint {
        return this.view().getBigInt64(index === undefined ? this.take(8) : this.at(index, 8), this.littleEndian());
    }
    ...
    public getFloat(index?: number): number {
        return this.view().getFloat32(index === undefined ? this.take(4) : this.at(index, 4), this.littleEndian());
    }
    public putFloat(value: number): ByteBuffer;
    public putFloat(index: number, value: number): ByteBuffer;
    public putFloat(first: number, second?: number): ByteBuffer {
    ...
    public getDouble(index?: number): number {
        return this.view().getFloat64(index === undefined ? this.take(8) : this.at(index, 8), this.littleEndian());
    }
    public putDouble(value: number): ByteBuffer;
    public putDouble(index: number, value: number): ByteBuffer;
    public putDouble(first: number, second?: number): ByteBuffer {
        this.view().setFloat64(
            second === undefined ? this.take(8) : this.at(first, 8),
            second ?? first,
            this.littleEndian(),
        );
        return this;
    ...
    public duplicate(): ByteBuffer {
        const copy = new ByteBuffer(this.bytes, this.positionValue, this.limitValue);
        copy.markValue = this.markValue;
        copy.byteOrderValue = this.byteOrderValue;
        return copy;
    }
    public slice(): ByteBuffer {
        const copy = new ByteBuffer(this.bytes.subarray(this.positionValue, this.limitValue), 0, this.remaining());
        copy.byteOrderValue = this.byteOrderValue;
        return copy;
    }
    public compact(): ByteBuffer {
        this.bytes.copyWithin(0, this.positionValue, this.limitValue);
        const remaining = this.remaining();
        this.positionValue = remaining;
        this.limitValue = this.capacity();
        this.markValue = -1;
        return this;
    }
    ...
    private signed(byte: number): number {
        return byte >= 128 ? byte - 256 : byte;
    }
    private take(width: number): number {
        if (width > this.remaining()) throw new IndexOutOfBoundsException("Buffer underflow or overflow");
        const index = this.positionValue;
        this.positionValue += width;
        return index;
    }
    private at(index: number, width: number): number {
        if (!Number.isInteger(index) || index < 0 || index > this.limitValue - width)
            throw new IndexOutOfBoundsException("Index outside buffer limit");
        return index;
    }
    ...
    private littleEndian(): boolean {
        return this.byteOrderValue === "LITTLE_ENDIAN";
    }
}
