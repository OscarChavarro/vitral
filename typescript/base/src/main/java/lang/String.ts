export class String {
    ...
    public substring(beginIndex: number, endIndex?: number): String {
        ...
        const safeEnd = endIndex === undefined ? sourceLength : endIndex > sourceLength ? sourceLength : endIndex;
    ...
    public indexOf(token: string | number, fromIndex = 0): number {
        ...
        const searchToken = typeof token === "number" ? globalThis.String.fromCharCode(token) : token;
    ...
    public static vformat(formatText: string, args: unknown[]): string {
        ...
        while (i < n) {
            ...
            switch (conversion) {
                ...
                case "c": {
                    text =
                        typeof arg === "number" ? globalThis.String.fromCharCode(arg) : String.argToText(arg).charAt(0);
                    break;
                }
                case "b":
                case "B": {
                ...
                default: {
                    text = formatText.slice(
                        i - 1 - flags.length - widthText.length - (hasPrecision ? precisionText.length + 1 : 0) - 1,
                        i,
                    );
                    argIndex--;
                    break;
