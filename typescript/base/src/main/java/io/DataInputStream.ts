...
export class DataInputStream {
    ...
    public readUnsignedByte(): number {
        ...
        if (value < 0) throw new RangeError("Unexpected end of stream");
        return value;
    }
    ...
    public readLong(): bigint {
        let value = 0n;
        for (let i = 0; i < 8; i++) value = (value << 8n) | BigInt(this.readUnsignedByte());
        return BigInt.asIntN(64, value);
    }
    public readFully(target: Uint8Array): void {
        let offset = 0;
        while (offset < target.length) {
            const count = this.input.readBytes(target, offset);
            if (count < 0) throw new RangeError("Unexpected end of stream");
            offset += count;
        }
    }
    public close(): void {
