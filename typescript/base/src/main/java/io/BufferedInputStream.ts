...
export class BufferedInputStream extends InputStream {
  ...
  public constructor(private readonly inputStream: InputStream, bufferSize = 8_192) {
    super();
  ...
  public override available(): number { this.requireOpen(); return (this.limit - this.position) + this.inputStream.available(); }
  public override mark(readLimit: number): void { this.requireOpen(); this.markLimit = globalThis.Math.max(0, readLimit); this.markPosition = this.position; }
  public override reset(): void { this.requireOpen(); if (this.markPosition < 0) throw new Error("Resetting to an invalid mark"); this.position = this.markPosition; }
