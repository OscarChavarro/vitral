...
export class ObjectOutputStream {
    ...
    public writeObject(value: unknown): void {
        const text = JSON.stringify(value, ObjectOutputStream.replacer);
        if (text === undefined) throw new TypeError("Value is not serializable");
    ...
    private static replacer(_key: string, value: unknown): unknown {
        return typeof value === "bigint" ? { $vitralBigInt: value.toString() } : value;
    }
}
