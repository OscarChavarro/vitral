...
export class ByteArrayInputStream extends InputStream {
    ...
    public override read(): number {
        return this.position >= this.limit ? -1 : (this.bytes[this.position++] ?? -1);
    }
    ...
    public override skip(count: number): number {
        const amount = globalThis.Math.min(globalThis.Math.max(0, count), this.limit - this.position);
        this.position += amount;
        return amount;
    }
    ...
    public override mark(_readLimit: number): void {
