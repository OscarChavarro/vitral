...
export class SimpleDateFormat {
  ...
  public format(date: Date | globalThis.Date): string {
    ...
    const replacements: Record<string, string> = {
      yyyy: part(value.getFullYear(), 4), yy: part(value.getFullYear() % 100, 2),
      MM: part(value.getMonth() + 1, 2), dd: part(value.getDate(), 2),
      HH: part(value.getHours(), 2), mm: part(value.getMinutes(), 2), ss: part(value.getSeconds(), 2),
