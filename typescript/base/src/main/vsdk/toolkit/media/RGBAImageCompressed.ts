...
export class RGBAImageCompressed extends Image {
    public static readonly COMPRESSION_UNKNOWN = 0;
    public static readonly COMPRESSION_DXT1 = 1;
    public static readonly COMPRESSION_DXT3 = 3;
    public static readonly COMPRESSION_DXT5 = 5;
    ...
    public setRawImage(w: number, h: number, f: number, d: Int8Array | Uint8Array): void {
    ...
    public getRawImageDirectBuffer(): Int8Array | null {
        return this.data;
    }
    ...
    public override getSizeInBytes(): number {
        return (this.data?.length ?? 0) + 24;
    }
    private unsupported(): never {
        throw new UnsupportedOperationException("Pixel access is unavailable for compressed image blocks");
    }
    public putPixel(_x: number, _y: number, _r: number | RGBAPixel, _g?: number, _b?: number, _a?: number): void {
        this.unsupported();
    }
    public putPixelRgb(_x: number, _y: number, _p: RGBPixel): void {
        this.unsupported();
    }
    public getPixel(_x: number, _y: number): RGBAPixel {
        return this.unsupported();
    }
    public getPixelRgb(_x: number, _y: number): RGBPixel;
    ...
    public static calculateTopLevelDataSize(w: number, h: number, f: number): number {
        ...
        if (f === this.COMPRESSION_DXT1) return blocks * 8;
        if (f === this.COMPRESSION_DXT3 || f === this.COMPRESSION_DXT5) return blocks * 16;
        return 0;
    }
}
