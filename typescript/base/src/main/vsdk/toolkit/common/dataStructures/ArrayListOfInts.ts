...
export class ArrayListOfInts extends FundamentalEntity {
    ...
    public constructor(increment: number) {
        ...
        this.array = new Int32Array(increment);
    }
    public size(): number {
        return this.mysize;
    }
    ...
    public sort(): void {
        if (this.mysize > 1) ArrayListOfInts.quicksort(this.array, 0, this.mysize - 1);
    }
    public static quicksort(array: Int32Array, left0: number, right0: number): void {
        let left = left0;
        let right = right0 + 1;
        ...
        array[right] = value;
        if (left0 < right) ArrayListOfInts.quicksort(array, left0, right);
        if (left < right0) ArrayListOfInts.quicksort(array, left, right0);
    }
    private grow(): void {
        const grown = new Int32Array(this.assignedSize + this.increment);
