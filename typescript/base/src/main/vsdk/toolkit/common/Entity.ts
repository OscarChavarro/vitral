...
export class Entity implements ModelElement {
  ...
  public getSizeInBytes(): number { return 0; }
  ...
  public getEncapsulatedVariables(): string[] {
    ...
    while (prototype !== null && prototype !== Entity.prototype) {
      for (const name of Object.getOwnPropertyNames(prototype)) {
        ...
        if (name.startsWith("get") && name.length > 3 && descriptor.value.length === 0) getters.set(name.slice(3), descriptor.value.bind(this) as () => unknown);
        if (name.startsWith("set") && name.length > 3 && descriptor.value.length === 1) setters.add(name.slice(3));
      }
  ...
  private static supportedTypeName(value: unknown): string | undefined {
    switch (typeof value) {
      ...
      case "object": return value === null ? undefined : (value as { constructor: { name: string } }).constructor.name;
