...
export class VSDK {
  ...
  public static equals(a: number, b: number): boolean { return Math.abs(a - b) < VSDK.EPSILON; }
  public static square(a: number): number { return a * a; }
  public static formatNumberWithinZeroes(a: number | bigint, n: number): string { return a.toString().padStart(n, "0"); }
  public static formatDouble(a: number): string;
  public static formatDouble(a: number, digits: number): string;
  public static formatDouble(a: number, digits = 2): string { return a.toFixed(digits); }
  ...
  public static formatIntAsHex(a: number): string { return (a >>> 0).toString(16).padStart(8, "0").toUpperCase(); }
  public static signedByte2unsignedInteger(input: number): number { return input < 0 ? input + 256 : input; }
  public static unsigned8BitInteger2signedByte(input: number): number {
    const value = Math.min(255, Math.max(0, input));
    return value > 127 ? value - 256 : value;
  }
  public static setWithSystemExit(flag: boolean): void { setWithSystemExit(flag); }
  public static setWithFatalExceptions(flag: boolean): void { setWithFatalExceptions(flag); }
}
