...
RendererConfiguration.prototype.compareTo = function(other: RendererConfiguration): number {
  const packed = (value: RendererConfiguration): number =>
    (value.isSurfacesSet() ? 1 : 0) + (value.isWiresSet() ? 2 : 0) +
    (value.isBoundingVolumeSet() ? 4 : 0) + (value.isSelectionCornersSet() ? 8 : 0) +
    (value.isTextureSet() ? 16 : 0) + (value.isBumpMapSet() ? 32 : 0) +
    (value.isPointsSet() ? 64 : 0) + (value.isNormalsSet() ? 128 : 0) +
    (value.isTrianglesNormalsSet() ? 256 : 0) + value.getShadingType() * 0x1000 +
...
RendererConfiguration.prototype.toString = function(): string {
  const shading = ["LIGHTING DISABLED (ONLY AMBIENT COLOR)", "FLAT", "GOURAUD", "PHONG", "COOK-TERRANCE"][this.getShadingType()] ?? "INVALID!";
