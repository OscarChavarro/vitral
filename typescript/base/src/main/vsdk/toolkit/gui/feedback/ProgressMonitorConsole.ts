...
export class ProgressMonitorConsole extends ProgressMonitor {
    ...
    public update(minValue: number, maxValue: number, currentValue: number): void {
        ...
        while (this.currentPercent + this.jumpPercent < value) {
            this.currentPercent += this.jumpPercent;
            if (!this.testLabelLimit(25) && !this.testLabelLimit(50) && !this.testLabelLimit(75)) this.write("-");
        }
    }
    public getCurrentPercent(): number {
        return this.currentPercent;
    }
    private testLabelLimit(limit: number): boolean {
        if (limit === this.lastPrintedLabel) return false;
        if (
            this.currentPercent - (6 * this.jumpPercent) / 10 < limit &&
            this.currentPercent + (6 * this.jumpPercent) / 10 > limit
        ) {
            this.write(` ${limit}% `);
            this.lastPrintedLabel = limit;
            return true;
        }
        return false;
    }
