...
import { StringTokenizer } from "../../../../java/util/StringTokenizer.js";
...
import type { ObjResourceProvider } from "./ObjResourceProvider.js";
...
export class ReaderObj extends PersistenceElement {
    ...
    other connection.
    */
    private static read(fileName: string, provider: ObjResourceProvider): TriangleMeshGroup {
        ...
        br = new BufferedReader(provider.openReader(fileName));

        while ((lineOfText = br.readLine()) !== undefined) {
            // Build material library
            if (lineOfText.startsWith("mtllib ")) {
                materialsHashMap = ReaderObj.readMaterials(lineOfText, fileName, provider);
            }
    ...
    */
    ...
    private static readMaterials(
        material: string,
        fileName: string,
        provider: ObjResourceProvider,
    ): HashMap<string, SimpleMaterial> {
        const ret = new HashMap<string, SimpleMaterial>();
        const st = new StringTokenizer(material, " ");
        st.nextToken(); // "mtlib" token
        let nomArc: string;
        nomArc = provider.resolveSibling(fileName, st.nextToken().toCString());

        try {
            const inReader = new BufferedReader(provider.openReader(nomArc));
            let lineOfText: string | undefined;
    ...
    public static importEnvironment(
        inSceneResourceName: string,
        inoutSimpleScene: SimpleScene,
        provider: ObjResourceProvider,
    ): void {
        //-----------------------------------------------------------------
        ...
        mg = ReaderObj.read(inSceneResourceName, provider);
