...
export async function sumWithWorkers(values: Int32Array, workers = navigator.hardwareConcurrency || 1): Promise<number> {
  const count = Math.max(1, Math.min(workers, values.length || 1));
  const executors = Array.from({ length: count }, () => new BrowserWorkerExecutor<SumInput, number>(new Worker(new URL("./sum.worker.js", import.meta.url), { type: "module" })));
  try {
...
sumWithWorkers(values).then((sum) => console.log({ count, workers: navigator.hardwareConcurrency || 1, sum, elapsedMilliseconds: performance.now() - startedAt }));
