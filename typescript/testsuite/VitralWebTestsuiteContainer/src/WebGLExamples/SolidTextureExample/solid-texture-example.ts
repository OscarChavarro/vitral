...
import { SolidTextureKeyboardInteractionTechniques } from './gui/solid-texture-keyboard-interaction-techniques';
import { SolidTextureMouseInteractionTechniques } from './gui/solid-texture-mouse-interaction-techniques';
import { TangibleInterfaceInteractionTechniques } from './gui/tangible-interface-interaction-techniques';
import { SolidTextureModel } from './model/solid-texture-model';
import { WebGLDebuggerRenderer } from './render/webgl-debugger-renderer';

/**
 * TypeScript/WebGL port of `java/testsuite/Jogl4Examples/SolidTextureExample`.
 *
 ...
 *
 * This is the module that exercises 3D textures and Perlin noise, and the
 * first with a heads-up display. The mesh it reads is only the shape the
 * procedural volume is carved into; what the keyboard bindings change is the
 * volume itself, and `[1]` switches to showing its slices directly.
 *
 ...
 *
 *   - `SolidTextureExample(String fileName)` reads its mesh from a
 *     `java.io.File`, asking `awt.FileSelectorDialog` for one when the command
 *     line named none. A browser has no file system, so the resource is always
 *     named by URL and the selector is the container's own modal dialog,
 *     reached by right-clicking `SolidTextureExample` in the explorer tree. The
 *     URL arrives here as the `meshUrl` input, and re-entering it reloads the
 *     scene, which is what re-running the Java program with another file does.
 ...
 */
@Component({
  selector: 'app-solid-texture-example',
  templateUrl: './solid-texture-example.html',
  styleUrl: './solid-texture-example.css',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SolidTextureExample implements OnChanges, OnDestroy {
   ...
   * value leaves `SolidTextureModel` its own default, as Java's setter does.
   */
  @Input()
  ...
  private readonly model = new SolidTextureModel();
  ...
  private readonly mouseInteractionTechniques = new SolidTextureMouseInteractionTechniques(
    this.cameraController,
  );
  private readonly keyboardInteractionTechniques = new SolidTextureKeyboardInteractionTechniques(
    this.model,
    this.cameraController,
    ...
    try {
      ...
      this.statusMessage.set(
        error instanceof Error
          ? error.message
          : 'WebGL solid texture example initialization failed',
      );
    }
  }
   ...
   * not let a page choose the depth-buffer precision. Java's `init` then
   * allocates buffers; this port compiles the renderer's shader programs
   * instead, which is why it is asynchronous.
   */
  private async initialize(): Promise<void> {
    const canvas = this.canvasRef.nativeElement;
    const gl = canvas.getContext('webgl2', { antialias: true, depth: true });
    if (gl === null) {
      throw new Error('SolidTextureExample requires WebGL2 support.');
    }
  ...
  private draw(): void {
    ...
    void this.renderLoop().catch((error: unknown) => {
      this.statusMessage.set(
        error instanceof Error ? error.message : 'WebGL solid texture example render failed',
      );
    });
  }
