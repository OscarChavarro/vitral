...
public class JoglTriangleMeshRenderer extends JoglRenderer {
    private static HashMap<TriangleMesh, TextureDecorator> objTex = 
        new HashMap<TriangleMesh, TextureDecorator> ();

    private static class TextureDecorator {
        TriangleMesh mesh;
        int[] textObjects;

        TextureDecorator(TriangleMesh m) {
            mesh = m;
        }
    }

    private static void initTexture(GL gl, TriangleMesh m) {
    ...
    */
    ...
    public static void drawSurfacesSolid(GL gl, TriangleMesh mesh, boolean flip) {
        JoglMaterialRenderer.activate(gl, mesh.getMaterial());
        for (int i = 0; i < mesh.getTriangles().length; i++) {
            gl.glBegin(gl.GL_TRIANGLES);
            {
    ...
    public static void drawWires(GL gl, TriangleMesh mesh) {
    ...
    public static void drawTexture(GL gl, TriangleMesh mesh) {
        JoglMaterialRenderer.activate(gl, mesh.getMaterial());
        int[] textObjs = null;
    ...
    private static void drawTextureRange(GL gl, TriangleMesh mesh, int ini, int fin,
                                         int texObj) {
    ...
    private static void drawSolidRange(GL gl, TriangleMesh mesh, int ini, int fin) {
        for (int i = ini; i < fin; i++) {
            gl.glBegin(gl.GL_TRIANGLES);
            {
                gl.glNormal3d(mesh.getVertexAt(mesh.getTriangleAt(i).getPoint0()).
    ...
    public static void drawBumpMap(GL gl, TriangleMesh mesh) {
        JoglMaterialRenderer.activate(gl, mesh.getMaterial());
    }
    ...
    public static void drawPoints(GL gl, TriangleMesh mesh) {
        JoglMaterialRenderer.activate(gl, mesh.getMaterial());
    ...
    public static void drawNormals(GL gl, TriangleMesh mesh) {
        for (int i = 0; i < mesh.getTriangles().length; i++) {
    ...
    public static void drawNormalsTriangles(GL gl, TriangleMesh m) {
    ...
    public static void drawWithSelection(GL gl, TriangleMesh mesh,
        ...
        if (quality.isWiresSet()) {
            drawWiresWithSelection(gl, mesh, selectedTriangles);
        }
        if (quality.isBoundingVolumeSet()) {
                //drawBoundingVolume(gl, mesh);
        }
    ...
    public static void drawWiresWithSelection(GL gl, TriangleMesh mesh,
                                              ArrayList<int[]> selectedTriangles) {
        ...
        for (int i = 0; i < mesh.getTriangles().length; i++) {
            ...
            for (int j = 0; j < selectedTriangles.size(); j++) {
                if (i == selectedTriangles.get(j)[1]) {
                    selected = true;
                }
            }
            if (selected)
