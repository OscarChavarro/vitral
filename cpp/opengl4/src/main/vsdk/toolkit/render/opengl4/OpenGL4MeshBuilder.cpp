...
OpenGL4MeshBuilder::OpenGL4MeshBuilder(double characteristicSize)
    : characteristicSize(characteristicSize), doubleSided(false)
{
}

void OpenGL4MeshBuilder::addTriangle(
...
void OpenGL4MeshBuilder::addQuad(
    const Vector3Dd& p0, const Vector3Dd& n0, double u0, double v0,
    const Vector3Dd& p1, const Vector3Dd& n1, double u1, double v1,
    const Vector3Dd& p2, const Vector3Dd& n2, double u2, double v2,
...
void OpenGL4MeshBuilder::addFrustum(double z0, double r0, double z1, double r1,
                                    int slices)
{
    double dz = z1 - z0;
    double dr = r0 - r1;
...
void OpenGL4MeshBuilder::addDisk(double z, double innerRadius,
...
void OpenGL4MeshBuilder::setDoubleSided(bool doubleSided)
{
    this->doubleSided = doubleSided;
}

OpenGL4MeshRenderer::Mesh* OpenGL4MeshBuilder::build()
{
    ...
    OpenGL4MeshRenderer::Mesh* mesh =
        new OpenGL4MeshRenderer::Mesh(characteristicSize);

    mesh->positions.resize((size_t)count * 3);
    mesh->normals.resize((size_t)count * 3);
    mesh->uvs.resize((size_t)count * 2);
    mesh->tangents.resize((size_t)count * 3);
    mesh->biNormals.resize((size_t)count * 3);
    for ( int i = 0; i < count; i++ ) {
        const float* v = &vertices[(size_t)i * VERTEX_SIZE];
        for ( int c = 0; c < 3; c++ ) {
            mesh->positions[i * 3 + c] = v[c];
            mesh->normals[i * 3 + c] = v[3 + c];
            mesh->tangents[i * 3 + c] = v[8 + c];
            mesh->biNormals[i * 3 + c] = v[11 + c];
        }
        mesh->uvs[i * 2] = v[6];
        mesh->uvs[i * 2 + 1] = v[7];
    }
...
void OpenGL4MeshBuilder::addVertex(const Vector3Dd& p, const Vector3Dd& n,
                                   double u, double v)
{
    Vector3Dd normal = n.normalized();
    Vector3Dd reference = std::fabs(normal.z()) < 0.9 ?
...
*/
void OpenGL4MeshBuilder::addBackSides(int frontCount)
{
