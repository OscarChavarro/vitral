...
namespace {
const float CAMERA_ORIGIN_COLOR[3] = { 1.0f, 0.5f, 0.5f };
const float FRUSTUM_COLOR[3] = { 0.0f, 1.0f, 1.0f };
}

Matrix4x4d OpenGL4CameraRenderer::activate(const Camera* cam)
{
    return cam->calculateProjectionMatrix();
}

Matrix4x4d OpenGL4CameraRenderer::activateCenter(const Camera* cam)
{
    Camera camera2(*cam);
    camera2.setPosition(Vector3Dd(0, 0, 0));
    camera2.setNearPlaneDistance(0.1);
    camera2.setFarPlaneDistance(10.0);
    return camera2.calculateProjectionMatrix();
}

void OpenGL4CameraRenderer::draw(const Camera* cam)
{
    Matrix4x4d projection = cam->calculateProjectionMatrix();
    draw(cam, projection);
}

void OpenGL4CameraRenderer::draw(const Camera* cam, const Matrix4x4d& projection)
{
    Matrix4x4d model = cam->getRotation().withTranslation(cam->getPosition());
    Matrix4x4d modelViewProjection = projection.multiply(model);
    drawVolume(cam, modelViewProjection);
}

void OpenGL4CameraRenderer::drawVolume(const Camera* cam, const Matrix4x4d& mvp)
{
    ...
    if ( cam->getProjectionMode() != Camera::PROJECTION_MODE_ORTHOGONAL ) {
        yn = 2 * npd * java::Math::tan(java::Math::toRadians(cam->getFov()) / 2);
    ...
    yf = yn;
    if ( cam->getProjectionMode() != Camera::PROJECTION_MODE_ORTHOGONAL ) {
        yf = 2 * fpd * java::Math::tan(java::Math::toRadians(cam->getFov()) / 2);
        xf = yf * (cam->getViewportXSize() / cam->getViewportYSize());
    }

    java::ArrayList<float> originPositions;
    java::ArrayList<float> originColors;
    ...
    java::ArrayList<float> frustumPositions;
    java::ArrayList<float> frustumColors;
    ...
    OpenGL4LineRenderer::drawLines(mvp, originPositions, originColors, 1.0f);
    OpenGL4LineRenderer::drawLines(mvp, frustumPositions, frustumColors, 2.0f);
}

void OpenGL4CameraRenderer::dispose()
{
    OpenGL4LineRenderer::release();
}

void OpenGL4CameraRenderer::addLoopRectangle(
...
void OpenGL4CameraRenderer::addLine(
    java::ArrayList<float>& positions,
    java::ArrayList<float>& colors,
    double x1,
    double y1,
...
void OpenGL4CameraRenderer::addVertex(
    java::ArrayList<float>& positions,
    java::ArrayList<float>& colors,
    double x,
    double y,
