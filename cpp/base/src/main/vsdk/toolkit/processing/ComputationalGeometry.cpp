...
ComputationalGeometry::TriangleIntersection* ComputationalGeometry::doIntersectionWithTriangle(const Ray& ray, const Vector3Dd& v0, const Vector3Dd& v1, const Vector3Dd& v2)
{
    ...
    TriangleIntersection* out = new TriangleIntersection();
    out->t = t;
...
int ComputationalGeometry::triangleContainmentTest(const Vector3Dd& p0, const Vector3Dd& p1, const Vector3Dd& p2, const Vector3Dd& p, double distanceTolerance)
{
    Vector3Dd q = closestPointOnTriangle(p, p0, p1, p2);
    if (q.subtract(p).length() <= distanceTolerance) return Geometry::LIMIT;
    return Geometry::OUTSIDE;
}

void ComputationalGeometry::triangleMinMax(const Vector3Dd& p0, const Vector3Dd& p1, const Vector3Dd& p2, double mm[6])
{
