...
void OpenGL1MinMaxRenderer::draw(Geometry* g, Camera* c, const Matrix4x4d& m) { if(g) draw(g->getMinMax(),c,m); }
void OpenGL1MinMaxRenderer::draw(const double* q, Camera* camera, const Matrix4x4d& m) {
    ...
    OpenGL1LineRenderer::drawLines(camera->calculateProjectionMatrix().multiply(m),p,c,1);
}
void OpenGL1MinMaxRenderer::dispose(){OpenGL1LineRenderer::release();}
