...
java::HashMap<RGBAImageCompressed*, GLuint> OpenGL1RGBAImageCompressedRenderer::compiledImages;
static java::ArrayList<GLuint> compiledTextureIds;

int OpenGL1RGBAImageCompressedRenderer::activate(RGBAImageCompressed* img) {
    if (img == nullptr) {
        return -1;
    }
...
void OpenGL1RGBAImageCompressedRenderer::deactivate(RGBAImageCompressed* img) {
    if (img != nullptr && compiledImages.containsKey(img)) {
        glBindTexture(GL_TEXTURE_2D, 0);
    }
}

void OpenGL1RGBAImageCompressedRenderer::unload(RGBAImageCompressed* img) {
    if (img == nullptr) {
        return;
    }
...
void OpenGL1RGBAImageCompressedRenderer::draw(RGBAImageCompressed* img) {
    if (img == nullptr) {
        return;
    }
...
GLuint OpenGL1RGBAImageCompressedRenderer::upload(RGBAImageCompressed* img) {
    if (img == nullptr) {
        return 0;
    }
    ...
    // OpenGL 1.2 has no compressed textures (glCompressedTexImage2D is
    // OpenGL 1.3): the image is always decoded in the CPU
...
java::ArrayList<unsigned char> OpenGL1RGBAImageCompressedRenderer::decompressToRGBA(const RGBAImageCompressed* img) {
...
int OpenGL1RGBAImageCompressedRenderer::readUShort(const unsigned char* data, int offset) {
    return ((data[offset] & 0xFF)) | ((data[offset + 1] & 0xFF) << 8);
}

int OpenGL1RGBAImageCompressedRenderer::readInt(const unsigned char* data, int offset) {
...
unsigned long long OpenGL1RGBAImageCompressedRenderer::readAlphaBits(const unsigned char* data, int offset) {
...
void OpenGL1RGBAImageCompressedRenderer::decodeRgb565(int packed, int& r, int& g, int& b) {
    r = ((packed >> 11) & 0x1F) * 255 / 31;
    g = ((packed >> 5) & 0x3F) * 255 / 63;
    b = (packed & 0x1F) * 255 / 31;
}

void OpenGL1RGBAImageCompressedRenderer::buildAlphaTable(int a0, int a1, int outTable[8]) {
    outTable[0] = a0;
    outTable[1] = a1;
    if (a0 > a1) {
...
void OpenGL1RGBAImageCompressedRenderer::disposeAll() {
