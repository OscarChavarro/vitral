...
const float OpenGL1RotateGizmoRenderer::ARC_OPACITY = 0.35f;

void OpenGL1RotateGizmoRenderer::draw(RotateGizmo* gizmo, Camera* camera)
{
...
void OpenGL1RotateGizmoRenderer::drawArc(const Matrix4x4d& mvp,
    const java::ArrayList<Vector3Dd>& fan, const ColorRgb& c)
{
    ...
    OpenGL1ColoredPrimitiveRenderer::draw(mvp, GL_TRIANGLE_FAN, positions, colors);
}

void OpenGL1RotateGizmoRenderer::drawRing(const Matrix4x4d& mvp,
    const java::ArrayList<Vector3Dd>& strip, const ColorRgb& c)
{
    ...
    OpenGL1ColoredPrimitiveRenderer::draw(mvp, GL_TRIANGLE_STRIP, positions, colors);
}
