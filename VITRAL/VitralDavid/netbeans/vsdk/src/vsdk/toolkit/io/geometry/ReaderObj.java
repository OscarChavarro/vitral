...
{
    private static final double PERTURBACION=0.0001;
    ...
    public static MeshGroup read(String fileName)throws IOException
    {
        ...
        while ( (cad = m.readLine()) != null)
        {
            ...
            if (cad.startsWith("o "))
            {
                Mesh object=armarObjeto(vertexes, normals, vertexesTex, faces, texturasList, relCarTex);
    ...
    private static RGBAImage obtenerTextura(String cad, String fileName)
    {
        ...
        try
        {
            return RGBAImageBuilder.buildImage(new File(dirObj+System.getProperty("file.separator")+nomImage));
        }
    ...
    private static Mesh armarObjeto(ArrayList<Vector3D> vertexes, ArrayList<Vector3D> normals,
            ...
            ArrayList<RGBAImage> texturas, ArrayList<ArrayList<int[]>> relCarTex)
    {
        ...
        m.setVerTexure(new Vector3D[usedTexVertexes.size()]);
 ...
 */
    ...
    private static Vector3D readVertex(String cad)
    {
        ...
        vert.x+=Math.random()*PERTURBACION;
        vert.y+=Math.random()*PERTURBACION;
        vert.z+=Math.random()*PERTURBACION;
