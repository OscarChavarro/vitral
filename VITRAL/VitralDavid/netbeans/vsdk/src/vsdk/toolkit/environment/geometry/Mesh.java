...
public class Mesh extends Geometry
{
    ...
    public void setTexTriRel(int[][][] texTriRel)
    {
        this.texTriRel = texTriRel;
    }
    ...
    public void calculateNormals()
    {
        ...
        /*
         * ArrayList<vsdk.toolkit.common.Triangle> [] vecinos; vecinos = new
         * ArrayList[vertexes.length]; for (int i = 0; i < vertexes.length; i++) {
         ...
         * 
         * for (int i = 0; i < vertexes.length; i++) { vertexes[i].setNormal(new
         * Vector3D(0, 0, 0)); for (int j = 0; j < vecinos[i].size(); j++) {
        ...
        vecinos = new ArrayList<ArrayList<vsdk.toolkit.common.Triangle>>(
                vertexes.length);
    ...
    /**
     * 
     * 0 - min (x) 1 - min (y) 2 - min (z) 3 - max (x) 4 - max (y) 5 - max (z)
     * 
     * @return double[]
     */
    ...
    {
        return ("Mesh < #T:" + triangles.length + " - #V" + vertexes.length
                + " - #N:" + " >");
    }
     ...
     * intersecta con la malla de triangulos. Si el rayo no intersecta al objeto
     * se retorna 0, y de lo contrario se retorna la distancia desde el origen
     * del rayo hasta el punto de interseccion.
     * 
     ...
     * del triangulo Referencia:
     * http://www.inmensia.com/articulos/raytracing/planotrianguloycubo.html
     */
    public boolean doIntersection(Ray inOut_Ray)
    {
        ...
        for (int i = 0; i < triangles.length; i++)
        {
            ...
            // vector normal a la superficie del plano que contiene al
            // trinangulo
            n = u.crossProduct(v);
            n.normalize();
            ...
            if (b != 0)
            {
                ...
                if (t < 0)
                    return false;
                // I = R(t)
                // punto de interseccion entre el rayo y el plano que contiene
                // el triangulo
                I = inOut_Ray.origin.add(inOut_Ray.direction.multiply(t));

                // verifica si el punto esta contenido dentro del triangulo
                double s1, s2, s3;
                s1 = (v1.substract(v0)).crossProduct(I.substract(v0))
                        .dotProduct(n);
                s2 = (v2.substract(v1)).crossProduct(I.substract(v1))
                        .dotProduct(n);
                s3 = (v0.substract(v2)).crossProduct(I.substract(v2))
                        .dotProduct(n);

                if ((s1 >= 0 && s2 >= 0 && s3 >= 0)
                        || (s1 <= 0 && s2 <= 0 && s3 <= 0))
                {
                    if (I.substract(inOut_Ray.origin).length() < _static_delta
                            .substract(inOut_Ray.origin).length())
                    {
