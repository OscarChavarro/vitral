...
public class JoglMeshRenderer
{
    ...
    public static void drawVertexNormal(GL gl, Vertex vertex)
    {
        ...
        gl.glVertex3d(vertex.getPosition().x, vertex.getPosition().y, vertex.getPosition().z);
        gl.glVertex3d(vertex.getPosition().x + (vertex.getNormal().x * 0.1), vertex.getPosition().y + (vertex.getNormal().y * 0.1),
        ...
        for(int i=0; i<mesh.getTriangles().length; i++)
        {
            gl.glBegin(gl.GL_TRIANGLES);
            {
                ...
                gl.glVertex3d(mesh.getVertexAt(mesh.getTriangleAt(i).getPoint2()).getPosition().x,
                        ...
                        mesh.getVertexAt(mesh.getTriangleAt(i).getPoint2()).getPosition().z);
            }
            gl.glEnd();
        }
    }
    ...
    public static void drawTexture(GL gl, Mesh mesh)
    {
        ...
        gl.glPushAttrib(gl.GL_ALL_ATTRIB_BITS);
        {
            ...
            for(int i=0; i<mesh.getTextTriRel().length && i<1; i++)
            {
                ...
                {
                    drawSolidRange(gl, mesh, mesh.getTextTriRelAt(i,j,0), mesh.getTextTriRelAt(i,j,1));
                }
            }
            ...
            for(int i=1; i<mesh.getTextTriRel().length; i++)
            {
                for(int j=0; j<mesh.getTexTriRelAt(i).length; j++)
                {
                    drawTextureRange(gl, mesh, mesh.getTextTriRelAt(i,j,0), mesh.getTextTriRelAt(i,j,1), textObjs[i-1]);
                }
            }
        }
        gl.glPopAttrib();
    ...
    private static void drawTextureRange(GL gl, Mesh mesh, int ini, int fin, int texObj)
    {
        ...
        for(int i=ini; i<fin; i++)
        {
            gl.glBegin(gl.GL_TRIANGLES);
            {
                ...
                gl.glTexCoord2d(mesh.getVerTextureAt(mesh.getTriangleAt(i).getVt0()).x, mesh.getVerTextureAt(mesh.getTriangleAt(i).getVt0()).y);
                ...
                gl.glTexCoord2d(mesh.getVerTextureAt(mesh.getTriangleAt(i).getVt1()).x, mesh.getVerTextureAt(mesh.getTriangleAt(i).getVt1()).y);
                ...
                gl.glTexCoord2d(mesh.getVerTextureAt(mesh.getTriangleAt(i).getVt2()).x, mesh.getVerTextureAt(mesh.getTriangleAt(i).getVt2()).y);
    ...
    public static void drawPoints(GL gl, Mesh mesh)
    {
        ...
        for (int i = 0; i < mesh.getVertexes().length; i++)
        {
            ...
            {
                gl.glVertex3d(mesh.getVertexAt(i).getPosition().x, mesh.getVertexAt(i).getPosition().y, mesh.getVertexAt(i).getPosition().z);
            }
            gl.glEnd();
        }
    }
    ...
    public static void drawNormalsTriangles(GL gl, Mesh m)
    {
        for(int i=0; i<m.getTriangles().length; i++)
        {
            ...
            gl.glBegin(gl.GL_LINES);
            {
                ...
                gl.glVertex3d(cx+m.getTriangleAt(i).normal.x*0.1, cy+m.getTriangleAt(i).normal.y*0.1, cz+m.getTriangleAt(i).normal.z*0.1);
            }
            gl.glEnd();
        }
    }
